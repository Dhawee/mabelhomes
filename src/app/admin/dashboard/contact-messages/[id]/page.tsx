...
export default function ContactMessageDetailPage({ params }: Props) {
  const { id } = use(params);
  ...
  const loadData = async () => {
    try {
      const msg = await api.get<ContactMessage>(`/api/contact-messages/${id}/`);
      setMessage(msg);

      const reps = await api.get<Reply[]>(`/api/contact-messages/${id}/replies/`);
      setReplies(reps);
    } catch (err: any) {
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/api/contact-messages/${id}/reply/`, {
  ...
  const handleMarkResponded = async () => {
    try {
      await api.patch(`/api/contact-messages/${id}/`, { status: "Responded", replied: true });
  ...
  const handleCloseEnquiry = async () => {
    try {
      ...
      alert(err.message || "Failed to close message.");
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/dashboard/contact-messages" className="btn btn-outline p-2">
            <ArrowLeft size={16} />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Contact Message #{message.id}</h1>
            <p className="text-xs text-gray-400">Received on {new Date(message.created_at).toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card card-body space-y-4">
            <div className="border-b border-gray-100 pb-3 flex justify-between items-center">
              <h2 className="font-bold text-gray-800 text-sm uppercase tracking-wide">Sender Details</h2>
              <span className={`badge ${message.status === "Pending" ? "badge-warning" : message.status === "Responded" ? "badge-success" : "badge-gray"}`}>
                {message.status}
              </span>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold text-blue-600">{message.email}</p>
              </div>
              <div>
                <p className="text-gray-400 text-xs">Phone Number</p>
                <p className="font-semibold text-gray-800">{message.phone || "N/A"}</p>
              </div>
              <div>
                <p className="text-gray-400 text-xs">Subject</p>
                <p className="font-semibold text-navy">{message.subject || "N/A"}</p>
              </div>
            </div>

            <div className="pt-3 border-t border-gray-100">
              <p className="text-gray-400 text-xs mb-1">Message Body</p>
          ...
          <div className="card card-body space-y-4">
            ...
            {replies.length > 0 ? (
              ...
              <p className="text-gray-400 text-sm italic py-2">No replies have been sent to this message yet.</p>
            )}
