...
import type { PropertyEnquiry } from "@/types/admin";
...
export default function PropertyEnquiryDetailPage({ params }: Props) {
  const { id } = use(params);
  ...
  const loadData = async () => {
    try {
      const enq = await api.get<PropertyEnquiry>(`/api/property-enquiries/${id}/`);
      setEnquiry(enq);

      const reps = await api.get<Reply[]>(`/api/property-enquiries/${id}/replies/`);
      setReplies(reps);
    } catch (err: any) {
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/api/property-enquiries/${id}/reply/`, {
        subject: `Re: ${enquiry.property_title}`,
        message: replyText,
      });
      setReplyText("");
  ...
  const handleDeleteEnquiry = async () => {
    if (!window.confirm(`Are you sure you want to PERMANENTLY delete enquiry #${id}? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      await api.delete(`/api/property-enquiries/${id}/`);
      alert("Enquiry deleted successfully.");
      router.push("/admin/dashboard/property-enquiries");
    } catch (err: any) {
  ...
  const handleCloseEnquiry = async () => {
    try {
      await api.patch(`/api/property-enquiries/${id}/`, { status: "Closed" });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/dashboard/property-enquiries" className="btn btn-outline p-2">
            <ArrowLeft size={16} />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Enquiry #{enquiry.id}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card card-body space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-gray-400 text-xs">Property Title</p>
                <p className="font-semibold text-gold">{enquiry.property_title}</p>
              </div>
            </div>
